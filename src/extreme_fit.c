...
void extremeFunction (double *parVector, double *xVector, int m, int n, void *adat);
void extremeDer (double *parVector, double *jacobian, int m, int n, void *adat);
...
int fitExtremeFunction(double* parMu, double* parSigma, const int N, double* pData, double* dData){

  double* parVector;
  double* xVector;
  double* jacobian;

  double parInitGuess[2] = {16.0,7.0};
  ...
  status=dlevmar_der(&extremeFunction,&extremeDer,parVector,jacobian,2,N,50,NULL,NULL,NULL,NULL,&dat);
  
  *parMu=parVector[0];
  *parSigma=parVector[1];
...
void extremeFunction (double *parVector, double *xVector, int m, int n, void *adat){
  ...
  double parMu = parVector[0];
  double parSigma = parVector[1];
  ...
  for (i = 0; i < n; i++){
    ...
    double pEst=exp(-1*exp((parMu-dDat[i])*parSigma));
...
void extremeDer (double *parVector, double *jacobian, int m, int n, void *adat){
  ...
  for (i=j=0; i < n; i+=1){
    ...
    jacobian[j++]=(-1)*exp(parSigma*(parMu-dDat[i])-exp(parSigma*(parMu-dDat[i])))*parSigma;
    jacobian[j++]=     exp(parSigma*(parMu-dDat[i])-exp(parSigma*(parMu-dDat[i])))*(dDat[i]-parMu);
